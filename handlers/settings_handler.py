...
async def create_paginated_keyboard(items, page, callback_prefix, buttons_per_row=2, back_callback="back_to_main_menu"):
    ...
    nav_row = []
    if page > 0:
        nav_row.append(InlineKeyboardButton("«", callback_data=f"{callback_prefix}page_{page-1}"))
    if end < len(items):
        nav_row.append(InlineKeyboardButton("»", callback_data=f"{callback_prefix}page_{page+1}"))
    if nav_row:
...
async def menu_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if route == 'back_to_main_menu':
        return await start_settings_conversation(update, context)
    if route == 'settings_close':
        await query.edit_message_text("Mode pengaturan ditutup.")
        return ConversationHandler.END
    if route == 'settings_packages':
        await query.edit_message_text("Silakan tempel (paste) daftar paket kustom Anda.\nKetik /cancel untuk batal.")
        return AWAITING_PACKAGES
    if route == 'settings_rootfs':
        await query.edit_message_text("Silakan kirim ukuran RootFS baru dalam MB.\nKetik /cancel untuk batal.")
        return AWAITING_ROOTFS_SIZE
    if route == 'settings_leech_dest':
        await query.edit_message_text("Silakan kirim ID Grup/Channel atau 'me'.\nKetik /cancel untuk batal.")
        return AWAITING_LEECH_DEST
    
    if route == "settings_version":
        await query.edit_message_text("Mengambil daftar versi...")
        versions = await scrape_openwrt_versions()
        if not versions:
            await query.edit_message_text("❌ Gagal Mengambil Data")
            return await start_settings_conversation(update, context)
        keyboard = []
        row = []
        ...
        await query.edit_message_text("Pilih seri rilis utama:", reply_markup=InlineKeyboardMarkup(keyboard))
        return SELECT_VERSION_MAJOR
    
    if route == "settings_target":
        version = bot_config.get("VERSION")
        await query.edit_message_text(f"Mengambil target untuk v{version}...")
        targets = await scrape_targets_for_version(version)
        if not targets:
            await query.edit_message_text(f"Tidak ada target untuk versi {version}.")
            return await start_settings_conversation(update, context)
        keyboard = await create_paginated_keyboard(targets, 0, "t_select_", 3, "back_to_main_menu")
    ...
    if route == "settings_profile":
        ...
        if not ib_filename:
            await query.edit_message_text("Gagal menentukan nama Image Builder. Pastikan Versi/Target valid.")
            return await start_settings_conversation(update, context)
        ...
        if not os.path.isdir(ib_dir):
            await query.edit_message_text(
                ...
                reply_markup=await create_main_settings_keyboard()
            )
            return MENU
...
async def select_profile_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data.startswith("p_select_page_"):
        ...
        profiles = await get_device_profiles(ib_dir)
        if not profiles:
             await query.edit_message_text("Gagal mendapatkan daftar profil.")
             return await start_settings_conversation(update, context)

        keyboard = await create_paginated_keyboard(profiles, page, "p_select_")
